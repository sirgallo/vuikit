...
export const usePathDataLoader = <T>(
  ...
  selectDataFn: (id: string) => Promise<T | undefined>,
) => {
  ...
  const selectNode = async (selectedId: string) => {
    try {
      ...
      const selectedData: Awaited<T> | undefined = await selectDataFn(selectedId)
      if (selectedData) {
        const selectedNode: PathNode<T> = {
          data: selectedData,
        ...
        if (selectedNode.refId && currNode.value?.id !== selectedNode.refId)
          await deselectNode(selectedNode.refId)
