...
const usePathDataLoaderDetailsList = ref([
  ...
  {
    key: 'extractPrevIdFn: (data: T) => string | null',
    content: 'function to extract reference to previous node in path',
  },
  {
    key: 'extractLinkedNodes: (data: T) => string[] | null',
    content: 'function to extract references to possible next path',
  },
  {
    key: 'selectDataFn: (id: string) => Promise<T>',
    content: 'function to asynchronously load data for next node in path',
  },
])
...
const useScrollLoaderDetailsList = ref([
  ...
  {
    key: 'scrollableElementRef: Ref<HTMLElement | null>',
    content: 'the scrollable html element to load content in',
  },
])
...
<template>
  <v-container orientation="vertical" overflow="auto">
    <v-container orientation="vertical" border>
      ...
      <v-list :items="useDebounceDetailsList" :extractKeyFn="(item: ListContent) => item.key">
    ...
    <v-container orientation="vertical" border>
      ...
      <v-list :items="useExpBackoffDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
    ...
    <v-container orientation="vertical" border>
      ...
      <v-list :items="useLocalStorageDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
    ...
    <v-container orientation="vertical" border>
      ...
      <v-list :items="useNavigateRouteDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
    ...
    <v-container orientation="vertical" border>
      <v-title title="usePathDataLoader" border />
      <v-list :items="usePathDataLoaderDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
      <v-input v-model:value="usePathDataLoaderImpl" disabled />
    </v-container>

    <v-container orientation="vertical" border>
      <v-title title="usePeriodicDataFetcher" border></v-title>
      <v-list :items="usePeriodicDataFetchDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
    ...
    <v-container orientation="vertical" border>
      ...
      <v-list :items="useScrollLoaderDetailsList" :extractKeyFn="(item: ListContent) => item.key">
        <template #content="{ content }">
          {{ content }}
        </template>
      </v-list>
