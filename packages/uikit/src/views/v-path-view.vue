...
const pathDetailListItems = ref([
  ...
  {
    key: 'extractIdFn: (data: T) => string',
    content: 'function for extract identifier from data for path node',
  },
  {
    key: 'extractPrevIdFn: (data: T) => string | null',
    content: 'function fo extract the reference to the previous node',
  },
  {
    key: 'extractLinkedNodes: (data: T) => string[] | null',
    content: 'function for extract links to potential selectable nodes',
  },
  {
    key: 'selectDataFn: (id: string) => Promise<T>',
    content: 'function for loading selected node in path',
  },
])
...
const extractLinkedNodes = (data: ShowcasePathNodeDataType) =>
  data?.linkMap ? Object.keys(data.linkMap) : null
const selectDataFn = async (id: string): Promise<ShowcasePathNodeDataType | undefined> =>
  data.value.find((el) => el.id === id)
...
<template>
  <v-container orientation="vertical" overflow="auto">
    <v-container orientation="vertical" border>
      ...
      <v-list :items="pathDetailListItems" :extractKeyFn="(item: ListContent) => item.key">
