...
const { items, loading } = useScrollLoader(props.loadKeysFn, masterKeyViewRef)
...
const observer = new ResizeObserver((entries) => {
  for (const entry of entries) {
    keyViewHeight.value = entry.target.scrollHeight
  }
})
  ...
  if (masterKeyViewRef.value) {
    ...
    watch(keyViewHeight, (height) => {
      if (masterViewRef.value && height) {
        if (!masterViewRef.value?.clientHeight || masterViewRef.value?.clientHeight >= height)
          updateHeight(Math.floor(height * 0.95))
      }
    })
  }
})
...
<template>
  <div ref="masterViewRef" class="v-master-view">
    <div ref="masterKeyViewRef" class="v-master-view-keys" :style="keyViewStyle">
      <div
        v-for="key of items"
        :key="extractKeyFn(key)"
        :class="
          selectedKey === key ? 'v-master-view-key v-master-view-selection' : 'v-master-view-key'
        "
      ...
      <font-awesome-icon v-if="loading" class="spinner" icon="fa-solid fa-spinner">
      </font-awesome-icon>
    </div>
