...
import { 
  type SearchEmits, type SearchProps, type SearchContextMap, type UsedSearchContextMap,
  defaultSearchProps
} from './v-search.types';
import { type ContextMap, useHints } from '../../composables/useHints.js'

const props = withDefaults(defineProps<SearchProps>(), defaultSearchProps)
const emit = defineEmits<SearchEmits>()
const { 
  findClosestContextStart,
  findWordStart,
  findWordEnd,
...
const parseCurrentContexts = (updatedValue: string): Pick<SearchContextMap, 'context' | 'filters'>[] => {
  ...
  while ((match = contextRegex.exec(updatedValue))) { // for each matched context, upate positions within the search string (start and end index for each context is tracked)
    if (match[1]) contextPositions.value.push({ context: match[1], start: match.index, end: match.index + match[1].length })
  }
...
const onHintClick = (hint: string) => {
  const joinStrings = (partialStrs: string[]): string => partialStrs.join(' ').trim().replace(/\s+/g, ' ').replace(/\s,/g, ',')
  const resolveInjectedEndInput = (wordEnd?: string): string => {
    if (! wordEnd || wordEnd.trim() === '') return ''
    if (! wordEnd.trim().startsWith(',') && ! wordEnd.trim().startsWith('[')) return `, ${wordEnd}`
    return wordEnd
  }

  const removeRepeatedWordsWithinPhrases = (input: string): string => {
    const phrases = input.split(',').map(phrase => {
  ...
  const currWordStart = findWordStart(textRef.value.slice(0, cursorRef.value), localContext.end, cursorRef.value, true)
  const currWordEnd = findWordEnd(textRef.value, currWordStart, cursorRef.value)

  const modifiedParts = [ textRef.value.slice(0, currWordStart), hint, resolveInjectedEndInput(textRef.value.slice(currWordEnd)) ]
  const modifiedString = removeRepeatedWordsWithinPhrases(joinStrings(modifiedParts))

  textRef.value = modifiedString
  refocusInput()
}
...
const refocusInput = () => {
  ...
  if (input) {
    input.focus()
    setTimeout(() => input.setSelectionRange(cursorRef.value, cursorRef.value), 0)
  }
}
